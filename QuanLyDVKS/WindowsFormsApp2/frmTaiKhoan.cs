...
namespace WindowsFormsApp2
{
    public partial class frmTaiKhoan : Form
    {
        ...
        public frmTaiKhoan()
        {
            ...
            guna2DataGridView1.DataError += Guna2DataGridView1_DataError; // THÊM SỰ KIỆN NÀY
        ...
        private void Guna2DataGridView1_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            try
            {
                ...
                if (colName == "Column3") // Password column
                {
                    // Always mask password column
                    e.Value = "••••••••";
                    e.FormattingApplied = true;
                }
                else if (colName == "Column4") // Type column
                {
                    // Xử lý an toàn cho cột Type
                    if (e.Value != null)
                    {
                        string valueStr = e.Value.ToString();
                        if (valueStr == "1" || string.Equals(valueStr, "Admin", StringComparison.OrdinalIgnoreCase))
                            e.Value = "Admin";
                        else if (valueStr == "0" || string.Equals(valueStr, "Staff", StringComparison.OrdinalIgnoreCase) ||
                                 string.Equals(valueStr, "Nhân viên", StringComparison.OrdinalIgnoreCase))
                            e.Value = "Nhân viên";
                        else
                            e.Value = "Nhân viên"; // Mặc định
                    }
                    else
                    {
                        e.Value = "Nhân viên"; // Mặc định khi null
                    }
                    e.FormattingApplied = true;
                }
            }
            ...
            {
                // Bỏ qua lỗi formatting, không làm crash ứng dụng
        ...
        private void LoadAccounts(string ma = null, string ten = null, string role = null)
        {
            try
            {
                ...
                using (var conn = Db.Open())
                using (var cmd = new SqlCommand(@"SELECT MaTaiKhoan, TenTaiKhoan, [Type]
                                              FROM dbo.TaiKhoan
                                              ...
                                              ORDER BY TenTaiKhoan", conn))
                {
                    ...
                    using (var rd = cmd.ExecuteReader())
                    {
                        while (rd.Read())
                        {
                            ...
                            // Password column - để trống, sẽ được format
                            row.Cells["Column3"].Value = "";

                            // Type column - chuyển đổi an toàn
                            var typeValue = rd["Type"];
                            if (typeValue != DBNull.Value && typeValue != null)
                            {
                                row.Cells["Column4"].Value = Convert.ToInt32(typeValue) == 1 ? "Admin" : "Nhân viên";
                            }
                            else
                            {
                                row.Cells["Column4"].Value = "Nhân viên";
                            }
                        }
                    }
                }
        ...
        private void Guna2DataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                // Đọc giá trị an toàn từ DataGridView
                txtMaTaiKhoan.Text = row.Cells["Column1"].Value?.ToString() ?? "";
                txtTenTaiKhoan.Text = row.Cells["Column2"].Value?.ToString() ?? "";

                // Không đọc password từ grid (bảo mật)
                txtMatKhau.Clear();

                // Đọc chức vụ an toàn
                var chucVuValue = row.Cells["Column4"].Value?.ToString() ?? "Nhân viên";
                txtChucVu.Text = chucVuValue;
