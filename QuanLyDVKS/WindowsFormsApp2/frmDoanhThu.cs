...
namespace WindowsFormsApp2
{
    public partial class frmDoanhThu : Form
    {
        ...
        {
            System.Windows.Forms.Cursor.Current = busy ? Cursors.WaitCursor : Cursors.Default;
            btnXem.Enabled = !busy;
            btnIn.Enabled = !busy;
        }
                                                ...
                                                WHERE hd.status = 1 AND hd.NgayLapHD >= @tu AND hd.NgayKetThucHD <= @den", conn))
                {
                    ...
                    using (var rd = cmd.ExecuteReader())
                    {
                        while (rd.Read())
                        {
                            ...
                            var tongHoaDon = Convert.ToDecimal(rd["TongTien"]);
                            var thanh = Convert.ToDecimal(rd["ThanhTien"]);
                            tienGiam += (tongHoaDon - thanh);
                            tongDV += tongHoaDon;
                            tongDoanhThu += thanh;
                        }
                    }
                                                  ...
                                                  ORDER BY SUM(ct.ThanhTien) DESC", conn))
                {
                    ...
                    using (var rd2 = cmd2.ExecuteReader())
                    {
                        ...
                        while (rd2.Read())
                        {
                            ...
                            if (idx < 8)
                            {
                                int pointIndex = series.Points.AddY((double)thanhTien);
                                var point = series.Points[pointIndex];
                                point.AxisLabel = ten;
                                point.LegendText = ten;
                                point.ToolTip = string.Format("{0:N0} VND", thanhTien);
                            }
                            idx++;
                        }
                    }
        ...
        // Added missing event handlers referenced by Designer
        private void label1_Click(object sender, EventArgs e)
        {
            // Focus the start-date picker when label is clicked
            dtpTuNgay.Focus();
        }
