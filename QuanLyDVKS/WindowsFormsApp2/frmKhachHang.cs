...
namespace WindowsFormsApp2
{
    public partial class frmKhachHang : Form
    {
        ...
        // Expose selected customer to caller (e.g., frmHome)
        ...
        public frmKhachHang()
        {
            ...
            lvKhachHang.DoubleClick += LvKhachHang_DoubleClick; // allow pick customer by double click
            ...
            // Ensure masks/maxlength allow10 digits for phone
        ...
        private void FrmKhachHang_Load(object sender, EventArgs e)
        {
            ...
            // allow 10-digit (formatted = 12 chars) and 11-digit (formatted = 13 chars)
            try { mtxtSDTKH.MaxLength = 14; } catch { }
        ...
        private void tsmiXoa1KH_Click(object sender, EventArgs e)
        {
            if (!_isAdmin) { MessageBox.Show("Bạn không có quyền truy cập"); return; }
            if (lvKhachHang.SelectedItems.Count == 0)
            {
                MessageBox.Show("Hãy chọn 1 khách hàng để xóa.");
                return;
            }
            int id;
            if (!int.TryParse(lvKhachHang.SelectedItems[0].SubItems[0].Text, out id))
            {
                MessageBox.Show("Không xác định được mã khách hàng.");
                return;
            }
            DeleteCustomersByIds(new System.Collections.Generic.List<int> { id });
        }

        private void tsmiXoaNhieuKH_Click(object sender, EventArgs e)
        {
            if (!_isAdmin) { MessageBox.Show("Bạn không có quyền truy cập"); return; }
            ...
            if (ids.Count == 0)
            {
                MessageBox.Show("Chọn ít nhất một khách hàng (checkbox) để xóa nhiều.");
                return;
            }
            DeleteCustomersByIds(ids);
        }

        private void DeleteCustomersByIds(System.Collections.Generic.List<int> ids)
        {
            // Loại bỏ KH có hóa đơn
            ids = ids.Where(id =>
            {
                if (QLKhachHang.HasInvoices(id))
                {
        ...
        // Chuẩn hóa nhập số điện thoại: chấp nhận10-11 số, không để trống giữa
        private void MtxtSDTKH_TextChanged(object sender, EventArgs e)
        {
            _dirtyInputs = true;
            // Lấy chỉ chữ số, cắt tối đa 10
            var digitsRaw = new string(mtxtSDTKH.Text.Where(char.IsDigit).ToArray());
            var digits = digitsRaw.Length >10 ? digitsRaw.Substring(0,10) : digitsRaw;

            // Không tự thêm khoảng trắng ở giữa, hiển thị nguyên dãy số
            int caret = mtxtSDTKH.SelectionStart;
            if (mtxtSDTKH.Text != digits)
            {
                mtxtSDTKH.Text = digits;
                mtxtSDTKH.SelectionStart = Math.Min(caret, mtxtSDTKH.Text.Length);
            }

            // Tô nền cảnh báo khi không hợp lệ: rỗng hoặc10-11 số thì OK
            mtxtSDTKH.BackColor = (digits.Length ==0 || Validators.IsValidVietnamPhone(digits))
        ...
        private void mtxtSDTKH_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
        ...
        private void LvKhachHang_DoubleClick(object sender, EventArgs e)
        {
            ...
            var it = lvKhachHang.SelectedItems[0];
            int id; if (!int.TryParse(it.SubItems[0].Text, out id)) return;
            SelectedCustomerId = id;
            SelectedCustomerName = it.SubItems.Count >1 ? it.SubItems[1].Text : null;
