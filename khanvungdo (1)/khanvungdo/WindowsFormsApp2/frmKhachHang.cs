...
namespace WindowsFormsApp2
{
    public partial class frmKhachHang : Form
    {
        ...
        private void MtxtSDTKH_TextChanged(object sender, EventArgs e)
        {
            ...
            var digits = digitsRaw.Length > 11 ? digitsRaw.Substring(0, 11) : digitsRaw;

            string formatted;
            if (digits.Length <= 10)
            {
                formatted = Regex.Replace(digits, @"(\d{0,4})(\d{0,3})(\d{0,3})",
                    m => string.Join(" ", new[] { m.Groups[1].Value, m.Groups[2].Value, m.Groups[3].Value }.Where(s => s.Length > 0)));
            }
            else
            {
                formatted = Regex.Replace(digits, @"(\d{0,4})(\d{0,3})(\d{0,4})",
                    m => string.Join(" ", new[] { m.Groups[1].Value, m.Groups[2].Value, m.Groups[3].Value }.Where(s => s.Length > 0)));
            }
            int caret = mtxtSDTKH.SelectionStart;
            mtxtSDTKH.Text = formatted;
