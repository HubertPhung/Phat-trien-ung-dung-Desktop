...
namespace WindowsFormsApp2
{
	public partial class frmDangNhap : Form
	{
        ...
        private void TxtTenTaiKhoan_TextChanged(object sender, EventArgs e)
        {
            ...
            // Autocomplete when length >= 2
            if (filtered.Length >= 2)
            {
                try
                {
                    using (var conn = Db.Open())
                    using (var cmd = new SqlCommand("SELECT TOP 5 TenTaiKhoan FROM dbo.TaiKhoan WHERE TenTaiKhoan LIKE @p ORDER BY TenTaiKhoan", conn))
                    {
                        cmd.Parameters.AddWithValue("@p", filtered + "%");
                        using (var rd = cmd.ExecuteReader())
                        {
                            var col = new AutoCompleteStringCollection();
                            while (rd.Read()) col.Add(rd.GetString(0));
                            txtTenTaiKhoan.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
                            txtTenTaiKhoan.AutoCompleteSource = AutoCompleteSource.CustomSource;
                            txtTenTaiKhoan.AutoCompleteCustomSource = col;
                        }
                    }
                }
                catch { }
