...
namespace ChuDe4
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            // Ensure the RestaurantManagement DB and its objects exist (runs once)
        ...
        private void BtnViewCurrentBill_Click(object sender, EventArgs e)
        {
            ...
            var dt = _db.ExecuteQuery("SELECT TOP 1 ID FROM dbo.Bills WHERE TableID=@TableID AND IsPaid=0 ORDER BY CheckIn DESC", prms);
            if (dt.Rows.Count == 0)
            {
